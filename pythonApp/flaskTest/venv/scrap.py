...
def createJsonTemplate(list, url):
    status = 200
    output = {"status": status, "list": list, "url": url }
    #res = json.dumps(output)
    return output


def get_scholar_list(url) -> str:
    ...
    if (page.status_code != 404):
        ...
        msg = createJsonTemplate(newy+newy2, url)
        print(msg)
        #mailserver(msg)
        return msg
    else:
